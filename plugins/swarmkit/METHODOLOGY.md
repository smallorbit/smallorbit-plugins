...
The reviewer is vendored into swarmkit (`plugins/swarmkit/agents/swarm-reviewer.md`) rather than imported from another plugin, so `/swarm` carries no cross-marketplace dependency. Worker scope is constrained: blockers and concerns are in scope, recommended coverage gaps are in scope, optional coverage gaps and nits are out of scope along with any unrelated cleanups. The worker may comment once on the PR summarizing what it addressed and what it deferred; the reviewer never posts comments.

Two flags tune the pass: `--reviewer-model <tier>` and `--worker-model <tier>`, both defaulting to `sonnet`.

The review/fix pass guarantees every swarm PR lands with at least one independent pass against its acceptance criteria — most valuable when the acceptance criteria in the originating issue are crisp enough for a reviewer to evaluate against. It is not a replacement for human review on the final merge — every PR still ends in the same place: open, awaiting your merge.
