...
`--team-name` is **not** a `spawn-team` flag — team naming is `--name` (above). It is the CLI flag the harness stamps on each spawned agent *subprocess*, and it matters only at teardown: `TeamDelete` retires the registry but leaves those processes running, so sweep them by matching it.

```bash
ps aux | grep -- "--team-name <team-name>" | grep -v grep | awk '{print $2}' | xargs -r kill
```
